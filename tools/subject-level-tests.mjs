...
function currentTopicsByLevel() {
  return {
    P3: ['汉语拼音 Hanyu Pinyin', '标点符号 Punctuation'],
    P4: ['词语运用 Vocabulary in Use'],
    P5: ['短文填空 Cloze Passage', '关联词 Connectives'],
...
test('THIS app knows which of the four it is', () => {
  ...
  eq(M.SUBJECT_KEY, 'chinese', 'SUBJECT_KEY is not this app');
...
test('a level narrows the prompt to THAT level\'s topics', () => {
  ...
  ok(p.indexOf('短文填空 Cloze Passage') >= 0, 'the P5 topics are not offered');
  ok(p.indexOf('关联词 Connectives') >= 0, 'the P5 topics are not all offered');
  ok(p.indexOf('汉语拼音 Hanyu Pinyin') < 0, 'a P3 topic is still on offer — the batch level does nothing');
  ok(p.indexOf('词语运用 Vocabulary in Use') < 0, 'a P4 topic is still on offer');
...
test('NO level leaves the prompt exactly as it always was', () => {
  ...
  ['汉语拼音 Hanyu Pinyin', '词语运用 Vocabulary in Use', '短文填空 Cloze Passage'].forEach(t =>
...
test('a level with no topics left falls back to the whole list', () => {
  ...
  ok(p.indexOf('汉语拼音 Hanyu Pinyin') >= 0, 'an empty level sent an empty topic list');
...
test('a topic already in the level is left alone', () => {
  const q = { topic: '短文填空 Cloze Passage', topicConfidence: 'high' };
  M._rapidApplyLevel(q, 'P5');
  eq(q.topic, '短文填空 Cloze Passage', 'a correct topic was overwritten');
...
test('an OFF-level topic is snapped into the level and flagged', () => {
  ...
  const q = { topic: '汉语拼音 Hanyu Pinyin', topicConfidence: 'high' };
  M._rapidApplyLevel(q, 'P5');
  eq(q.topic, '短文填空 Cloze Passage', 'an off-level topic survived the batch level');
...
test('a topic on NO list is snapped too', () => {
  ...
  M._rapidApplyLevel(q, 'P5');
  eq(q.topic, '短文填空 Cloze Passage', 'an unknown topic was kept');
...
test('a SECONDARY topic above the level is dropped', () => {
  ...
  const q = { topic: '短文填空 Cloze Passage', topic2: '阅读理解 Comprehension' };
  M._rapidApplyLevel(q, 'P5');
  eq(q.topic2, '', 'an out-of-level secondary topic survived');
  eq(q.topic, '短文填空 Cloze Passage', 'the primary topic was disturbed');
});

test('a secondary topic INSIDE the level is kept', () => {
  const q = { topic: '短文填空 Cloze Passage', topic2: '关联词 Connectives' };
  M._rapidApplyLevel(q, 'P5');
  eq(q.topic2, '关联词 Connectives', 'a valid secondary topic was thrown away');
});

test('no level chosen changes nothing at all', () => {
  const q = { topic: '汉语拼音 Hanyu Pinyin', topic2: '阅读理解 Comprehension', topicConfidence: 'high' };
  M._rapidApplyLevel(q, '');
  eq(q.topic, '汉语拼音 Hanyu Pinyin', 'an unlevelled batch was re-filed');
  eq(q.topic2, '阅读理解 Comprehension', 'an unlevelled batch lost its secondary topic');
...
test('a level with no topics behind it never re-files a question', () => {
  ...
  eq(q.topic, '汉语拼音 Hanyu Pinyin', 'a question was filed into an empty level');
...
test('the snap survives a question with no topic at all', () => {
  const q = {};
  M._rapidApplyLevel(q, 'P5');
  eq(q.topic, '短文填空 Cloze Passage', 'a topicless question was left topicless');
});
